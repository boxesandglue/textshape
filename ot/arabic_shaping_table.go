...
// Arabic Shaping Table for Fallback Shaping
//
// This table maps Arabic codepoints (U+0621-U+06D3) to their Unicode
// Presentation Forms (U+FE70-U+FEFF and U+FB50-U+FDFF).
//
// Source: HarfBuzz hb-ot-shaper-arabic-table.hh:236-417
//
// Column order: [init, medi, fina, isol]
// This matches the arabic_fallback_features order in HarfBuzz.

// shapingTableFirst is the first codepoint in the shaping table.
const shapingTableFirst = 0x0621

// shapingTableLast is the last codepoint in the shaping table.
const shapingTableLast = 0x06D3

// shapingTable maps Arabic codepoints to Presentation Forms.
// Index: codepoint - shapingTableFirst
// Columns: [init, medi, fina, isol]
var shapingTable = [179][4]uint16{
	...
	{0x0000, 0x0000, 0x0000, 0x0000}, // U+0640 ARABIC TATWEEL
	...
	{0xFBE8, 0xFBE9, 0xFEF0, 0xFEEF}, // U+0649 ARABIC LETTER ALEF MAKSURA
	...
	{0x0000, 0x0000, 0x0000, 0xFBDD}, // U+0677 ARABIC LETTER U WITH HAMZA ABOVE
...
// Feature index constants for shapingTable columns
// These match arabic_fallback_features order in HarfBuzz
const (
	shapingFeatureInit = 0 // Initial form
	shapingFeatureMedi = 1 // Medial form
	shapingFeatureFina = 2 // Final form
	shapingFeatureIsol = 3 // Isolated form
)

// getShapingEntry returns the presentation form for a codepoint and feature.
// Returns 0 if not found or no mapping exists.
func getShapingEntry(cp Codepoint, featureIndex int) Codepoint {
	if cp < shapingTableFirst || cp > shapingTableLast {
		return 0
	}
	if featureIndex < 0 || featureIndex > 3 {
		return 0
	}
	return Codepoint(shapingTable[cp-shapingTableFirst][featureIndex])
}
